...
function shouldSkipCopy(name, fullPath) {
  const rel = path.relative(ROOT, fullPath).split(path.sep)[0];
  return EXCLUDED_ROOTS.has(rel) || EXCLUDED_FILES.has(name);
}

function copySite(src, dest) {
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    const srcPath = path.join(src, entry.name);
    if (shouldSkipCopy(entry.name, srcPath)) continue;

    const destPath = path.join(dest, entry.name);
    if (entry.isDirectory()) {
      fs.mkdirSync(destPath, { recursive: true });
      copySite(srcPath, destPath);
...
function relativeHref(fromFile, targetUrl) {
  const fromDir = path.dirname(path.relative(OUT, fromFile));
...
function main() {
  ...
  for (const file of htmlFiles) {
    ...
    const linked = autolinkHtml(html, file, terms);
    fs.writeFileSync(file, linked);
  }

  console.log(`Built ${path.relative(ROOT, OUT)} with glossary autolinks across ${htmlFiles.length} HTML files.`);
}
